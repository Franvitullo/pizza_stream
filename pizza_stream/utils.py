...
def load_config(config_path: str = "config.json") -> dict:
    ...
    if not os.path.isabs(config_path):
        # Asumir que está en el mismo directorio que este script si es relativo
...
def _which_or_raise(name: str, executable_path: str = None) -> str:
    # 1. Si se provee una ruta explícita (desde config), probamos esa primero.
    if executable_path and executable_path != name:
        if os.path.isfile(executable_path):
            return executable_path
        # Si la ruta explícita parece un comando global (sin path), dejamos que shutil.which lo busque.
        # Pero si tiene separadores de directorio y no existe, es un error.
        if os.sep in executable_path or (os.altsep and os.altsep in executable_path):
             print(f"Warning: Configured path '{executable_path}' for '{name}' not found. Falling back to PATH.")

    # 2. Buscar en el PATH del sistema
    exe = shutil.which(executable_path if executable_path else name)
    if not exe:
        # Fallback: intentar buscar 'name' si executable_path falló
        if executable_path and executable_path != name:
             exe = shutil.which(name)
        
    if not exe:
         raise FileNotFoundError(
            f"No se encontró '{name}' (ruta config: '{executable_path}'). "
            f"Asegúrate de tenerlo instalado y en el PATH, o configura la ruta completa en config.json."
        )
    return exe


def _ffprobe_duration(input_path: str) -> float:
    # Intentamos deducir ffprobe desde la config si es posible, o asumimos PATH
    # Por simplicidad, cargamos config aquí o asumimos 'ffprobe' en PATH si no se pasa nada.
    # Idealmente, deberíamos pasar la config o la ruta.
    # Para no romper firmas, cargaremos config localmente.
    try:
        config = load_config()
        ffmpeg_path = config.get("ffmpeg_path", "ffmpeg")
        # Si ffmpeg_path es una ruta completa, asumimos que ffprobe está al lado
        if os.path.isabs(ffmpeg_path) and "ffmpeg" in os.path.basename(ffmpeg_path).lower():
            ffprobe_name = os.path.basename(ffmpeg_path).lower().replace("ffmpeg", "ffprobe")
    ...
    ffprobe = _which_or_raise("ffprobe", ffprobe)
...
def sincronizar_audio_video(input_path: str, output_path: str, desfase_ms: int) -> str:
    ...
    # Validamos/buscamos el ejecutable real para prevenir FileNotFoundError en subprocess
    ffmpeg_exe = _which_or_raise("ffmpeg", ffmpeg_bin)
...
def remove_silence_precise(
    ...
    config = load_config()
    ffmpeg_bin_conf = config.get("ffmpeg_path", "ffmpeg")
    ffmpeg_bin = _which_or_raise("ffmpeg", ffmpeg_bin_conf)
    
    # Intentar deducir ffprobe
    if os.path.isabs(ffmpeg_bin) and "ffmpeg" in os.path.basename(ffmpeg_bin).lower():
         ffprobe_name = os.path.basename(ffmpeg_bin).lower().replace("ffmpeg", "ffprobe")
         ffprobe_path = os.path.join(os.path.dirname(ffmpeg_bin), ffprobe_name)
    else:
         ffprobe_path = "ffprobe"
         
    _which_or_raise("ffprobe", ffprobe_path)
...
def convert_videos_vertical(
    ...
    for in_path in paths:
        ...
        # Cargar config para obtener ffmpeg path
        try:
             conf = load_config()
             ffmpeg_bin_conf = conf.get("ffmpeg_path", "ffmpeg")
             ffmpeg_exe = _which_or_raise("ffmpeg", ffmpeg_bin_conf)
        except:
             ffmpeg_exe = "ffmpeg" # Fallback si falla carga
